...
public abstract class DBeani2 implements DBean {
	...
	private void init(Class<?> filter) {
		...
		while (orderpf == null || orderpf == inull) {
			orderpf = DBeani2.allprofile.putIfAbsent(this.dbclz, inull);
			if (orderpf != null && orderpf != inull) {
				int lsz = orderpf.size();
				//int lszci = (lsz <= 4) ? lsz : lsz * 100 / 75;
				ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>> cq = DBeani2.getCacheQ(lsz);
				HashMap<L<?>, H> pred = null;
				while (pred == null) {
					SoftReference<HashMap<L<?>, H>> rfmp = cq.poll();
					if (rfmp == null) {
						pred = new HashMap<L<?>, DBeani2.H>((lsz <= 4) ? lsz : lsz * 100 / 75);
						for (L<?> i : orderpf) {
							pred.put(i, new H());
						}
					} else {
				...
				this.data = pred;
				DBeani2.occupiedRes.put(new WeakReference<DBeani2>(this, DBeani2.clearedNoteQ), pred);
				return;
	...
	private void setup(Class<?> filter) {
		...
		DBeani2.allprofile.replace(this.dbclz, thefis);
		return;
	...
	private static ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>> getCacheQ(int size) {
		ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>> cq = DBeani2.cacheQ.get(size);
		ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>> xq;
		if (cq == null) {
			xq = new ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>>(DBeani2.cacheQsize);
			cq = DBeani2.cacheQ.putIfAbsent(size, xq);
			if (cq == null)
				cq = xq;
		}
		return cq;
	...
	public static class ConfBeani extends DBeani2 implements ConfBean {
	...
	static class Rc implements Runnable {
		...
		public void run() {
			while (true) {
				try {
					Reference<? extends Object> ref = DBeani2.clearedNoteQ.remove();
					if (ref instanceof WeakReference<?>) {
						HashMap<L<?>, H> res = DBeani2.occupiedRes.remove(ref);
						for (H i : res.values()) {
							i.s = null;
						}
						ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>> cq = DBeani2.getCacheQ(res.size());
						cq.offer(new SoftReference<HashMap<L<?>, H>>(res, DBeani2.clearedNoteQ));
					} else if (ref instanceof SoftReference<?>) {
						for (ArrayBlockingQueue<SoftReference<HashMap<L<?>, H>>> i : DBeani2.cacheQ.values()) {
							i.remove(ref);
						}
					}
